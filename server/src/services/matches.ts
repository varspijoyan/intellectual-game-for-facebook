...
import { getQuestionById, pickRandomQuestionId, rowToDto, type QuestionDto } from "./questions.js";
...
export async function joinAsyncMatch(db: Db, matchId: number, playerId: string) {
  ...
  if (players.some((p: any) => p.fb_player_id === playerId)) {
...
export async function getMatchDto(db: Db, matchId: number): Promise<MatchDto | null> {
  ...
  const players: MatchPlayerDto[] = rows.map((r: any) => ({
...
export async function listMatchesForPlayer(
  ...
  let q = db("matches")
    ...
    .select("matches.id", "matches.status", "matches.mode", "matches.context_id")
...
export async function submitAnswer(
  ...
  playerId: string,
  answerIndex: number
): Promise<{ correct: boolean; score: number; match: MatchDto; question: QuestionDto | null } | { error: string }> {
